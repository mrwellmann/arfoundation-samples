...
public class IndicatorViewer : MonoBehaviour
{
    ...
    [Tooltip("How many seconds before indicators update their tracking. Higher = better performance, but more stuttering. Can not be changed at runtime")]
    [Range(0, 1)]
    public float UpdateInterval = 0.015f;
    [Tooltip("Update indicators every frame regardless of Update Interval value")]
    public bool UpdateEveryFrame = true;
    [Tooltip("The farthest distance indicators will reach from the screen center to the screen edges. Align slider with TargetEdgeDistance for seamless transition. Notation is TRBL (like in css)")]
    public Vector4 IndicatorEdgeDistance = new Vector4(0.95f, 0.95f, 0.95f, 0.95f);
    [Tooltip("The farthest distance the target needs to reach from the screen center to the screen edges to be considered off-screen. Align slider with IndicatorEdgeDistance for seamless transition. Notation is TRBL (like in css)")]
    public Vector4 TargetEdgeDistance = new Vector4(0.95f, 0.95f, 0.95f, 0.95f);
    ...
    [Tooltip("The minimum and maximum X and Y scale size of the indicator. Only Active if AutoScale = true. Set minimum Value to adjust actual Indicator Size.")]
    ...
    void OnEnable()
    {
        if (UpdateEveryFrame)
        {
            Application.onBeforeRender += UpdateRendering;
        }
        else
        {
