...
public class IndicatorTarget : MonoBehaviour
{
    ...
    private bool isVisible;                     //  Check if the viewer is on-screen or off-screen
    ...
    private void Awake()
    {
        ...
        if (viewer == null)
        {
            Debug.LogError("IndicatorTarget: IndicatorViewer Object is missing or not active");
        }
    }
    ...
    private void InitializeIndicator()
    {
        ...
        //Debug.Log("IndicatorTarget: viewer.DefaultIndicatorPanel = " + CustomIndicatorPanel);
        ...
        try
        {
            ...
            //panel.transform.SetParent(viewer.transform);
    ...
    #endregion Initial set-up of indicator
    ...
    #endregion Update the position & rotation of the indicator panel of this target.
    ...
    private bool OnScreen(Vector3 pos, Camera camera)
    {
        ...
        if (pos.x < (Screen.width * viewer.TargetEdgeDistance.w) && pos.x > (Screen.width - Screen.width * viewer.TargetEdgeDistance.y) &&
            pos.y < (Screen.height * viewer.TargetEdgeDistance.x) && pos.y > (Screen.height - Screen.height * viewer.TargetEdgeDistance.z) &&
            pos.z > camera.nearClipPlane && pos.z < camera.farClipPlane)
            return true;
        return false;
    }

    #endregion Method that returns true if target is within the camera screen boundaries and it's near clipping plane.
    ...
    #endregion Method that returns false if indicator does not need to be disabled because either DisableOnDistance is false or distance is within threshold.
    ...
    {
        // Clamp this to prevent distance 0
        distance = Mathf.Clamp(distance, 0.01f, 100);
    ...
    #endregion Method that updates scale
    ...
    private void UpdateOffScreen(Vector3 targetPosOnScreen)
    {
        ...
        //  Adapted to non-uniform screen boundaries - Original version commented below. Syntax is TRBL (like in css)
        //  Create the screen boundaries that the indicators reside in.
        Vector4 screenBounds = new Vector4(screenCenter.y * viewer.IndicatorEdgeDistance.x, screenCenter.x * viewer.IndicatorEdgeDistance.y, screenCenter.y * viewer.IndicatorEdgeDistance.z, screenCenter.x * viewer.IndicatorEdgeDistance.w);

        //  Check which screen side the target is currently in.
        //  Check top & bottom
        if (cos > 0)
            newIndicatorPos = new Vector2(-screenBounds.x / m, screenBounds.x);
        else
            newIndicatorPos = new Vector2(screenBounds.z / m, -screenBounds.z);

        //  Check left & right
        if (newIndicatorPos.x > screenBounds.y)
            newIndicatorPos = new Vector2(screenBounds.y, -screenBounds.y * m);
        else if (newIndicatorPos.x < -screenBounds.w)
            newIndicatorPos = new Vector2(-screenBounds.w, screenBounds.w * m);
    ...
    #endregion Calculate OffScreen Position, Rotation, and Size
    ...
    #endregion that returns the distance magnitude between two vector3 positions. (Faster then built-in Vector3.Distance function)
    ...
    #endregion OnScreen Tranisitions
    ...
    #endregion OffScreen Transitions
    ...
    #endregion Method that handles destroying of indicator
